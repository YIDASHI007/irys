...
async def spritetype(account, proxy, run_count, run_mode):
    ...
    while i < run_count:
        ...
        time_val = 15
        total_chars = int(wpm * 5 * time_val / 60)  # 对应 Math.floor(wpm * 5 * time / 60)
        incorrect_chars = int(random.random() * max(1, total_chars // 15))  # 对应 Math.floor(Math.random() * Math.max(1, totalChars / 15))
        correct_chars = total_chars - incorrect_chars
        accuracy = 100 if total_chars == 0 else round(100.0 * correct_chars / total_chars)  # 对应 Math.round(100.0 * correctChars / totalChars)
        
        progress_data = []
        for j in range(time_val):
            base_val = correct_chars * (j + 1) / time_val
            jitter = int(random.random() * 3) - 1  # 对应 Math.floor(Math.random() * 3) - 1
        ...
        anti_cheat_hash = compute_anti_cheat_hash(account, wpm, accuracy, time_val, correct_chars, incorrect_chars)
        ...
        show_msg(f"address={account}, wpm={wpm}, accuracy={accuracy}, time={time_val}, correctChars={correct_chars}, incorrectChars={incorrect_chars}, antiCheatHash={anti_cheat_hash}")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    show_msg(f"HTTP错误: {response.status_code} - {json.dumps(error_data)}", 2)
                except:
            ...
            # 如果是400错误，显示请求数据用于调试
            if hasattr(err, 'response') and err.response and err.response.status_code == 400:
